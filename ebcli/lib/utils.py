...
from ebcli.resources.constants import OSSpecific, ServiceDefault, \
    TerminalConstant
...
def is_os_windows():
    if 'nt' in sys.builtin_module_names \
            or get_os_name() == OSSpecific.WINDOWS_NAME:
        return True
    else:
        return False
...
def string_to_boolean(string):
    ...
    if string_equal_ignore_case(string, TerminalConstant.Y) \
        ...
        raise AttributeError(u'Not recognized boolean value: '
                             u'"{0}".'.format(string))


def bool_to_yesno(value):
    if not isinstance(value, bool):
        raise AttributeError(u'"{0}" is not instance of boolean.'
                             .format(value))
    if value:
