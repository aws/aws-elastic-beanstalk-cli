...
# from ebcli.core.exception import EBCliException
#
#
...
#     else:
#         raise EBCliException('Parameter must be instance of list or string.')
#
#     app.log.debug('Running external commands "{0}".'.\
#               format(misc.collection_to_string(command_line)))
#     # Using OS native code page
#     command_line = map(lambda x: x.encode(locale.getpreferredencoding()), command_line)
...
#                     return branch.split(u' ')[1], len(branches)
#
#     except subprocess.CalledProcessError as ex:
#         # Git returned with an error code
#         log.error(u'Git local repository is not set up:  "{0}".'.format(ex))
#         if quiet:
#             return None, 0
#         else:
...
#         log.error(u'Encountered an error when copying file from {0} to {1}, because: {2}'.\
#                   format(src, dst, ex))
#         if not quiet:
#             raise
#
