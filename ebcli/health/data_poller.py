...
class DummyDataPoller(DataPoller):

    def _get_health_data(self):
        environment_health = self. \
            get_environment_health(self.env_name)
        instance_health = self.get_instance_health(self.env_name)
        LOG.debug('EnvironmentHealth-data:{}'.format(environment_health))
        LOG.debug('InstanceHealth-data:{}'.format(instance_health))
        token = instance_health.get('NextToken', None)
        ...
        instance_health = collapse_instance_health_data(instance_health)
        while token is not None:
            h = elasticbeanstalk.get_instance_health(self.env_name,
                                                     next_token=token)
            token = h.get('NextToken', None)
            instance_health += collapse_instance_health_data(h)
    ...
    def get_environment_health(self, env_name):
        return dict(HealthStatus='Ok', EnvironmentName='dummy-data',
                    Color='Green',
                    ApplicationMetrics={
                        u'Duration': 10,
                        u'Latency': {
                        ...
                        u'StatusCodes': {
                            ...
                            u'Status_3xx': 200,
                            u'Status_2xx': 7640}},
                    RefreshedAt=datetime.now(tz.tzutc()),
                    InstancesHealth={u'Info': 10, u'Ok': 10, u'Unknown': 10,
                                     u'Severe': 10, u'Warning': 10,
                                     u'Degraded': 10, u'NoData': 10,
                                     u'Pending': 10},
                    Causes=[
                        '0.1 % of the requests to the ELB are failing (5 minutes ago)',
                        'No data for 10 out of 80 instances',
                        '12.5 % of the requests are failing (HTTP 5xx)'
                    ])


    def get_instance_health(self, env_name):
        possible_causes = [
            '95 % of CPU is in use',
            'Command CMD-SelfStartup has been running for 26 seconds',
            '6.7 % of the requests are failing (HTTP 5xx)',
        ...
        for i in range(0, 80):
            instance = {}
            status, color = random.choice(possible_statuses)
            def rand_latency():
                n = random.randrange(100000) / 100.0
                star = random.choice(['', '', '', '', '*'])
                return '{}{}'.format(n, star)
            instance[u'HealthStatus'] = status
            instance[u'Color'] = color
            instance[u'InstanceId'] = 'i-' + ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(8))
            instance[u'ApplicationMetrics'] = {
                    u'Duration': 10,
                    u'Latency': {u'P99': rand_latency(), u'P75': rand_latency(),
                                 u'P90': rand_latency(), u'P95': rand_latency(),
                                 u'P85': rand_latency(), u'P10': rand_latency(),
                                 u'P999': 0.819,
                                 u'P50': 0.269},
                    u'RequestCount': 100,
                    u'StatusCodes': {u'Status_5xx': 1,
                                     u'Status_4xx': 3,
                                     u'Status_3xx': 1,
                                     u'Status_2xx': 95}}
            instance[u'System'] = {u'Loadavg': [0.29, 0.26, 0.12],
                                   u'CPUUtilization': {u'Softirq': 0.0, u'Iowait': 0.1,
                                                       u'System': 0.2, u'Idle': 99.3,
                                                       u'User': 0.4, u'Irq': 0.0,
                                                       u'Nice': 0.0}}
            instance[u'Color'] = 'Green'
            instance[u'LaunchedAt'] = datetime(2015, 4, 29, 18, 20, 1,
                                             tzinfo=tzutc())
            num_of_causes = random.choice([0,1,1,2,3])
            causes = []
            for i in range(num_of_causes):
                causes.append(random.choice(possible_causes))
            instance[u'Causes'] = causes
            instances.append(instance)
        return dict(InstanceHealthList=instances)