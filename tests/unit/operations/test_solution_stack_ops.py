...
class TestSolutionstackOps(unittest.TestCase):
	...
	@mock.patch('ebcli.operations.solution_stack_ops.detect_platform')
	def test_prompt_for_solution_stack__cli_determined_language_for_customer(
			self,
			solution_stack_detector
	):
		matching_language_names = [
			'Ruby',
			'Node.js',
			'Tomcat'
		]
		solution_stack_detector.return_value = 'Ruby'

		self.assertEqual(
			'Ruby',
			solution_stack_ops.prompt_for_language_name(matching_language_names)
		)

	@mock.patch('ebcli.lib.utils.prompt_for_index_in_list')
	@mock.patch('ebcli.operations.solution_stack_ops.detect_platform')
	def test_prompt_for_solution_stack__module_name_is_none(
			self,
			solution_stack_detector,
			index_prompter_mock
	):
		solution_stack_detector.return_value = None
		matching_language_names = [
			'Ruby',
			'Node.js',
			'Tomcat'
	...
	@mock.patch('ebcli.lib.utils.prompt_for_index_in_list')
	@mock.patch('ebcli.operations.solution_stack_ops.detect_platform')
	def test_prompt_for_solution_stack__module_name_is_passed(
			self,
			solution_stack_detector,
			index_prompter_mock
	):
		...
		self.assertEqual(
			'Node.js',
			solution_stack_ops.prompt_for_language_name(matching_language_names, 'my-module')
		)
	...
	@mock.patch('ebcli.operations.solution_stack_ops.detect_platform')
	def test_detect_platform__language_determined__customer_approves(
			self,
			language_detector_mock,
			boolean_response_generator_mock
	):
		language_detector_mock.return_value = 'Node.js'
		boolean_response_generator_mock.return_value = True

		self.assertEqual(
			'Node.js',
			solution_stack_ops.detect_platform()
		)

	@mock.patch('ebcli.core.io.get_boolean_response')
	@mock.patch('ebcli.lib.heuristics.find_language_type')
	def test_detect_platform__language_determined__customer_disapproves(
			self,
			language_detector_mock,
			boolean_response_generator_mock
	):
		language_detector_mock.return_value = 'Node.js'
		boolean_response_generator_mock.return_value = False

		self.assertIsNone(solution_stack_ops.detect_platform())
	...
	def test_detect_platform__language_determined__customer_approves(
		...
		language_detector_mock.return_value = None
	...
	def test_get_solution_stack_from_customer__custom_platforms_absent__customer_chooses_platform_without_multiple_versions(
			self,
			platforms_lister_mock,
			input_prompter_mock,
			solution_stacks_lister_mock,
			boolean_response_mock
	...
	@mock.patch('ebcli.lib.utils.prompt_for_index_in_list')
	@mock.patch('ebcli.operations.platformops.list_custom_platform_versions')
	def test_get_solution_stack_from_customer__custom_platforms_absent__customer_chooses_platform_with_multiple_versions(
			self,
			platforms_lister_mock,
			input_prompter_mock,
			solution_stacks_lister_mock,
		...
		available_solution_stacks = [
			...
			SolutionStack('64bit Amazon Linux 2017.09 v2.6.0 running PHP 7.0'),
		]
	...
	def test_get_solution_stack_from_customer__choose_custom_platform_option(
			self,
			solution_stack_detector_mock,
			custom_platform_versions_retriever_mock,
			index_prompter_mock,
		...
		index_prompter_mock.side_effect = [
			2,
			1,
			2
		]
