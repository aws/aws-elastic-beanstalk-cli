...
class PlatformWorkspaceTests(TestHelpTexts):
    def setUp(self):
        disable_socket()
...
commands:
  ...
  list              Lists versions of the custom platform associated with this workspace.
...
Lists versions of the custom platform associated with this workspace.
...
optional arguments:
  ...
  -s STATUS, --status STATUS
                        the status that you wish to filter on (Ready, Failed,
                        Deleting, Creating)

You can reduce the result set by using filters.
""".strip(),
...
class PlatformWorkspaceEBPTests(TestHelpTexts):
    def setUp(self):
        disable_socket()
        self.root_dir = os.getcwd()
        if not os.path.exists('testDir'):
...
class ApplicationWorkspaceTests(TestHelpTexts):
    def setUp(self):
        disable_socket()
        self.root_dir = os.getcwd()
        if not os.path.exists('testDir'):
...
Shows a list of platforms for use with "eb init -p". Type "--verbose" to get the full platform name.
""".strip(),
            self.output_of(['eb', 'platform', 'list', '--help'])
        )
