...
from ebcli.docker import containerops
...
class TestPreconfiguredContainer(TestCase):
    ...
    @patch('ebcli.docker.preconfigured_container._validate_preconfig_dockerfile')
    def test_no_preconfig_dockerfile_check(self, _validate_preconfig_dockerfile):
        _bypass_preconfig_dockerfile_validation(self.fs_handler)
    ...
    @patch('ebcli.docker.preconfigured_container.containerops._get_preconfig_info')
    @patch('ebcli.docker.preconfigured_container.commands._get_base_img')
    def test_validation_pass(self, _get_base_img, _get_preconfig_info):
        _expect_preconfig_dockerfile_validation(self.fs_handler)
    ...
    @patch('ebcli.docker.preconfigured_container.Container._get_log_volume_map')
    ...
    @patch('ebcli.docker.preconfigured_container.log')
    @patch('ebcli.docker.preconfigured_container.containerops')
    @patch('ebcli.docker.preconfigured_container.Container._get_log_volume_map')
    def test_get_log_volume_map_exists_by_default_log_path(self,
        ...
        log.get_host_log_path.return_value = HOST_LOG_PATH
        ...
        self.assertDictEqual(self.container._get_log_volume_map(), expected_volume_map)


def _bypass_preconfig_dockerfile_validation(fs_handler):
    fs_handler.dockerfile_exists = False


def _expect_preconfig_dockerfile_validation(fs_handler):
    fs_handler.dockerfile_exists = True
